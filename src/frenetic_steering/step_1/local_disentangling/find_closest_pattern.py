...
from frenetic_steering.application_on_images.util import number_of_neurons_with_different_value

def find_closest_pattern(input):
    distances = []
    for i in range(load_images.get_number_of_patterns()):
        pattern = load_images.load_pattern(i)
        distances.append(number_of_neurons_with_different_value(input, pattern))
